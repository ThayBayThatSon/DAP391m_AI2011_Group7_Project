...
\end{equation}
The 215-hour boundary guard for long-term forecasting is determined by combining the maximum spatial and day-prior lag lookback window (191 hours) with a 24-hour forecast horizon safety margin. Audit checks verify that target proxies and future features are excluded.
...
XGBoost leads in overall $R^2$ (0.4648) and RMSE (19.3007), followed closely by LightGBM and Linear Ridge. Notably, Linear Ridge achieves the lowest MAE at the 24-hour horizon (13.5179). Persistence drops to $R^2 = 0.2307$, reflecting substantial 24-hour AQI shifts.
...
Vu et al.\ achieved a temporal cross-validation $R^2$ of 0.8500 and an out-of-bag $R^2$ of 0.9200 by integrating high-resolution GOES-16 satellite Aerosol Optical Depth (AOD), HRRR meteorology, and SMOTE oversampling. In our benchmark, our top short-term nowcasting model (XGBoost) achieves a comparable test $R^2$ of 0.8711 (and Random Forest achieves $R^2 = 0.8528$), demonstrating a qualitative comparison where tabular meteorological attributes paired with autoregressive AQI lags achieve comparable near-term performance despite different evaluation scopes and dataset conditions.
...
Removing VPD alters $R^2$ by $< 0.003$ across settings. VPD acts as an interpretable dryness descriptor rather than a standalone accuracy driver. Spatial lags show mixed impact: removing them slightly improves short-term $R^2$ while reducing long-term MAE (improving long-term MAE by 0.0286). Furthermore, the long-term configuration uses 49 features compared to 47 in short-term nowcasting because local short-term lags ($t-1$ to $t-23$) are replaced by expanded multi-station spatial aggregates and day-prior contextual indicators.
...
\item \textbf{Code availability}: The code, data processing pipelines, and reproducibility materials are available in an anonymous repository for double-blind peer review at \url{https://anonymous.4open.science/r/AQI-California-Benchmark-2025}.
