...
\titlerunning{Comparative US AQI Time Series Forecasting in California}
\author{Ta Nguyen Anh Minh \and Thu Le \and Trinh Quoc Sang}
\authorrunning{T. N. A. Minh \and T. Le \and T. Q. Sang}
\institute{FPT University, Ho Chi Minh City, Vietnam \\
\email{taminh123xt@gmail.com, thulvm@fpt.edu.vn, trinhquocsang1@gmail.com}}
\maketitle

\begin{abstract}
Forecasting hourly US Air Quality Index (AQI) in California is difficult, because the series reacts to urban emissions, regional meteorology, marine influence, and seasonal wildfire smoke transport. The task is highly autocorrelated in the short term but highly volatile during upper-tail pollution episodes, so a model evaluated only on global test-year metrics may overestimate true operational reliability. We propose a rigorous time-series benchmark using a merged panel of 181,122 observations from 2018--2025, evaluated under a climate-context-aware temporal split that isolates the extreme 2020 season for validation. Five tabular regressors are compared against two naive baselines under short-term nowcasting and strict 24-hour-ahead forecasting configurations. 
Against these baselines, XGBoost recovers a strong advantage in the short term, reaching an $R^2$ of 0.8711, and leads the 24-hour horizon with an $R^2$ of 0.4648. However, extreme-event scenario evaluation shows that machine learning models struggle to reliably anticipate rare upper-tail AQI behavior beyond simply copying the most recent observation. An ablation shows that engineered vapor pressure deficit (VPD) and spatial AQI lags provide only modest standalone improvements to global metrics. Every number is produced by executed code.

\keywords{US AQI forecasting \and PM2.5 \and California \and time-series regression \and climate-context evaluation \and vapor pressure deficit \and spatial lag \and LightGBM \and extreme-event analysis}
\end{abstract}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.58\linewidth]{figures/paper_station_map.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{figures/paper_correlation_heatmap_en.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\linewidth]{figures/07_vpd_monthly_diagnostic.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.82\linewidth]{figures/paper_workflow_pipeline_architecture.jpg}
...
Five machine learning models are evaluated: Linear Ridge, Random Forest, LightGBM, CatBoost, and XGBoost. Linear Ridge provides a regularized linear baseline. Random Forest represents bagging-based nonlinear learning. LightGBM, CatBoost, and XGBoost represent gradient-boosted decision-tree families. CatBoost receives categorical station/source identifiers natively; the other models use train-only preprocessing for categorical encoding and scaling where applicable.

Additionally, two naive operational baselines are included to ground the results:
\begin{itemize}
    \item \textbf{Persistence}: Predicts the AQI will remain exactly the same as the most recent available observation (e.g., $t-1$ for short-term nowcasting, and $t-24$ for long-term forecasting).
    \item \textbf{Climatology}: Predicts the historical mean AQI for a given hour-of-day and month, learned from the training set.
\end{itemize}
...
XGBoost obtains the highest short-term $R^2$ at 0.8711, while also obtaining the lowest MAE. All models, including the Persistence baseline, exceed 0.85 in $R^2$, indicating that recent AQI history is highly informative for near-term nowcasting. The fact that the naive Persistence model performs better than Random Forest and nearly matches CatBoost emphasizes the extreme strength of short-term autocorrelation. This result should be interpreted as evidence of strong temporal persistence rather than as evidence that the complex models fully understand atmospheric mechanisms. The leakage audit reports a 0.927 correlation between the 2025 target and the one-hour AQI lag, which explains why even Linear Ridge and Persistence remain highly competitive. Climatology, however, fails completely in this task as it cannot capture short-term transient pollution spikes.
...
The long-term task is significantly harder because local AQI observations from $t-1$ to $t-23$ are excluded. XGBoost achieves the highest $R^2$ (0.4648), but its advantage over LightGBM and Linear Ridge is small. Unlike the short-term task, the Persistence baseline collapses to an $R^2$ of 0.2307, demonstrating that AQI conditions change considerably over 24 hours. The models must therefore rely on smoother day-prior structure, seasonal timing, spatial lags, and meteorological context to outperform Persistence. Climatology again produces negative $R^2$, indicating that using long-term historical means is worse than simply predicting the global mean. Abrupt upper-tail events remain difficult to anticipate from the available tabular features alone.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{figures/eda_aqi_category_timeline.png}
...
Short-term & January 2025 Winter Extreme-AQI Window & Linear Ridge & 1795 & 9.2156 & 14.2454 & 0.8473 \\
...
Long-term & January 2025 Winter Extreme-AQI Window & LightGBM & 1726 & 19.4994 & 32.7758 & 0.2103 \\
...
The table clarifies three patterns. First, short-term nowcasting remains strong for the full year, the non-event baseline, the January winter window, and the July--September seasonal slice because recent AQI persistence is still available. Notably, the Persistence baseline emerges as the \emph{best} performer in the short-term Extreme AQI (top 5\%) and Wildfire Season slices, further underscoring that machine learning models struggle to reliably learn complex atmospheric drivers of extremes beyond simply copying the most recent observation. Second, the top-5\% AQI slice is much harder even in the short-term setting, with the best $R^2$ falling to 0.3004. Third, the 24-hour top-tail scenario produces negative $R^2$ across all models, with Persistence being the ``least bad'' at -2.3742. This means that even the best models perform worse than a scenario-specific mean predictor on this restricted subset. This does not contradict the global leaderboard; rather, it shows that rare upper-tail AQI behavior is not sufficiently represented by weather, VPD, spatial lags, and day-prior AQI alone.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.78\linewidth]{figures/paper_lightgbm_predicted_vs_actual.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.74\linewidth]{figures/06_extreme_aqi_events_heatmap_year_month.png}
...
Several limitations should be considered when interpreting the results. First, \texttt{target\_aqi} is a PM2.5-derived AQI proxy, not a complete regulatory AQI label derived from all criteria pollutants. Second, the dataset combines EPA station observations with Open-Meteo supplemental records, which may introduce source heterogeneity. Notably, due to Open-Meteo providing records up to December 2025 and EPA AQS records typically lagging in real-time, the final year contains primarily Open-Meteo samples, posing an out-of-distribution risk. Third, the 2025 test-year distribution is affected by this available supplemental source alignment, so station-level generalization should be interpreted carefully. Fourth, VPD and spatial lags are physically motivated engineered features, but the ablation study shows limited standalone evidence of improvement. Fifth, the extreme-event section identifies temporal clustering only; without external fire and smoke observations, it cannot attribute high-AQI records to wildfire causes.

\section{Conclusion and Future Work}
This paper presented a California US AQI forecasting benchmark with climate-context and extreme-event evaluation. Using 181,122 hourly observations from 2018--2025, the study compared Linear Ridge, Random Forest, LightGBM, CatBoost, and XGBoost under short-term nowcasting and 24-hour forecasting scenarios. XGBoost achieved the best $R^2$ in both settings, with 0.8711 for short-term nowcasting and 0.4648 for long-term forecasting.

The results show that near-term AQI prediction is strongly driven by temporal persistence, while 24-hour and upper-tail AQI forecasting remain difficult when recent local AQI is hidden. The fact that the naive Persistence baseline performs as the best model for extreme AQI windows underscores the challenge. The VPD ablation result is especially important for academic wording: VPD is best described as an engineered dryness feature, not as a proven standalone performance driver. Spatial lags provide mixed evidence, occasionally showing small performance degradations when included. Future work should integrate event-aware data such as fire perimeters, smoke plumes, aerosol optical depth, planetary boundary-layer height, and upwind transport indicators. It should also extend ablation testing across model families and feature groups, and evaluate uncertainty-aware warning thresholds for high-AQI conditions.
