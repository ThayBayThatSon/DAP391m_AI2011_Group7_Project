...
\maketitle
\fontsize{12.5pt}{16.5pt}\selectfont
...
We deployed trained pipelines into a real-time early warning web application. A FastAPI backend handles model inference and computes SHAP values for feature interpretability. An SQLite database caches Open-Meteo weather fields and EPA AQS data to minimize latency.
...
Figure~\ref{fig:xgboost-predicted-actual} plots XGBoost predictions against actual AQI. While central values match closely, high-AQI peaks display dispersion.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.82\linewidth]{figures/paper_xgboost_predicted_vs_actual.png}
    \caption{Predicted versus actual AQI for XGBoost on the independent 2025 test year.}
    \label{fig:xgboost-predicted-actual}
\end{figure}
